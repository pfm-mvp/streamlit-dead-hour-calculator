...
def normalize_vemcount_response(response_json: dict) -> pd.DataFrame:
    ...
    # Detect structuurtype
    # ✅ Nieuw: buitenste laag zijn datums (date_2025-08-07)
    if all(k.startswith("date_") for k in response_json.keys()):
        for date_key, shops in response_json.items():
            for shop_id, shop_content in shops.items():
                dates = shop_content.get("dates", {})
                for hour_key, hour_info in dates.items():
    ...
    # ✅ Oud formaat: buitenste laag zijn shop_ids
    elif all(k.isdigit() for k in response_json.keys()):
